...
input long   InpMagicNumber       = 260406;
...
//|                                                                  |
//+------------------------------------------------------------------+
bool IsAcGreen(const double &ac[], int shift)
  {
// 绿色：AC[i] > AC[i+1]，仅比较已收盘柱（避免用到 shift=0）
   return (ac[shift] > ac[shift + 1]);
  }
...
int OnInit()
  {
   ...
   acHandle = iAC(InpSymbol, Tf);

   if(maFastHandle == INVALID_HANDLE || maSlowHandle == INVALID_HANDLE || atrHandle == INVALID_HANDLE || acHandle == INVALID_HANDLE)
      return(INIT_FAILED);
...
// AC 变色判定：在已收盘柱 shift=1 上发生颜色切换
// 绿色：AC[shift] > AC[shift+1]；红色反之
   bool acGreen1 = IsAcGreen(ac, 1);
   bool acGreen2 = IsAcGreen(ac, 2);
   bool acRed1 = !acGreen1;
   bool acRed2 = !acGreen2;

   bool longTrigger = (ac[1] < 0.0) && acGreen1 && acRed2;
   bool shortTrigger = (ac[1] > 0.0) && acRed1 && acGreen2;

// AC 极值（语义：变色柱往前数第二根已收盘柱）
// 在 Series 下，往“更旧”方向是 +，因此用 shift=3（相对变色柱 shift=1 往前两根）
   double acExtreme = ac[3];
...
// 同向只开一笔（仅看本 EA 的持仓）
   ...
   if(allowLong && !hasBuy && maBull && longTrigger && (acExtreme < InpAcExtremeLong))
     {
...
// 多单止损：慢线下方 InpSlAtrMult 倍 ATR
      double sl = NormalizePrice(maSlow[1] - InpSlAtrMult * atrVal);
   ...
   if(allowShort && !hasSell && maBear && shortTrigger && (acExtreme > InpAcExtremeShort))
     {
